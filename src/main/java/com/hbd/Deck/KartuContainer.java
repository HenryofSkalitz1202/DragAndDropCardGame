...
public class KartuContainer implements Iterable<Kartu> {
    ...
    public Kartu peekKartuAt(int index) throws DeckOutOfBoundsException, DeckEmptyException {
        if (size() == 0) {
            throw new DeckEmptyException("Deck Kosong");
        }
        if (index < 0) {
            throw new DeckOutOfBoundsException("Tidak bisa mengambil pada posisi negatif");
        }
        if (index > size()) {
            throw new DeckOutOfBoundsException("Jumlah Deck tidak mencukupi");
        }
    ...
    public void insertBanyakKartu(List<Kartu> banyakKartu) throws DeckPenuhException {
        if (size() + banyakKartu.size() > capacity && capacity != -1) {
            throw new DeckPenuhException("Deck sudah penuh");
        }
    ...
    public void print() {
        ...
        // If there are fewer than 6 cards, print additional nulls to ensure 6 slots are
        // shown
